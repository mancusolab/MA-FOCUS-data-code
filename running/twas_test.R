...
for ( w in 1:nrow(wgtlist) ) {
  ...
  if ( length(cur.Z) == 0 ) {
    ...
    if ( sum(cur.miss) != 0 ) {
      if ( sum(!cur.miss) == 0 ) {
        ...
        r2pred <- cur.wgt %*% cur.LD[!cur.miss,!cur.miss] %*% t(cur.wgt)
        # print(paste0("min:", min(r2pred)))
        # print(paste0("max:", max(r2pred)))
        # print(paste0("mean:", mean(r2pred)))
        # print(paste0("median:", median(r2pred)))
        # print(cur.Z[!cur.miss])
        # print(cur.Z[cur.miss])
        
        all.r2pred = rep(1,length(cur.Z))
        all.r2pred[ cur.miss ] = cur.r2pred
        # print(paste0("r2pred: ",  mean( all.r2pred[ wgt.matrix[,mod.best] != 0 ] ) ))
  ...
  # hack to solve names(topeqtl) is null problem
  if ( cur.FAIL || sum(eqtlmod) == 0 || length(topeqtl) == 0 || is.na(topeqtl) || is.null(names(topeqtl))) {
