...
genoa_ea <- read_tsv("./data/genoa_hr_ea.tsv", col_names = FALSE)
colnames(genoa_ea) <- c("GENE", "GENEID", "POP", "HSQ", "HSQSE", "HSQP", "VG", "VGSE")
mean(genoa_ea$VG)
tidy(t.test(genoa_ea$VG))

genoa_aa <- read_tsv("./data/genoa_hr_aa.tsv", col_names = FALSE)
...
mean(tot$geuv_yri_s, na.rm = TRUE)
mean(tot$geuv_eur_s, na.rm = TRUE)
...
mean(tot$geuv_eur, na.rm = TRUE)/mean(filter(geuvadis_all, POP == "EUR")$HSQ)
mean(tot$geuv_yri, na.rm = TRUE)/mean(filter(geuvadis_all, POP == "YRI")$HSQ)

mean(tot$geuv_eur, na.rm = TRUE)/mean(filter(geuvadis_all, POP == "EUR")$VG)
mean(tot$geuv_yri, na.rm = TRUE)/mean(filter(geuvadis_all, POP == "YRI")$VG)
