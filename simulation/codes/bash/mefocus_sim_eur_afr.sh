...
start_time=$SECONDS
# params=`sed "${NR}q;d" ${SCRIPTF}/param_eur_afr.tsv`
# params=`sed "${NR}q;d" ${SCRIPTF}/param_eur_afr_3pop.tsv`
params=`sed "${NR}q;d" ${SCRIPTF}/param_eur_afr_real.tsv`
echo "${NR} ${params}"
set -- junk $params
shift
...
# TMP=/scratch/zeyunlu/tmp/mefocus/eur_afr/
TMP=/scratch/zeyunlu/tmp/mefocus/eur_afr_real2/
# TMP=/scratch/zeyunlu/tmp/mefocus/eur_afr_3pop/

echo "Running on Locus ${LOCUS} on Row ${ROW}"

OUT=$TMP/row${ROW}.locus${LOCUS}

python ${SCRIPTF}/mefocus_run_eur_afr.py \
