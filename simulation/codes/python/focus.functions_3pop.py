...
def compute_ld(G):
    ...
    return LD, L, ldscs, mafs, p


def gen_ld(prefix_pop1, prefix_pop2, prefix_pop3):
    # read in plink data
    bim_1, fam_1, G_1 = read_plink(prefix_pop1, verbose=False)
    bim_2, fam_2, G_2 = read_plink(prefix_pop2, verbose=False)
    bim_3, fam_3, G_3 = read_plink(prefix_pop3, verbose=False)

    tmpsnps = pd.merge(bim_1, bim_2, how="inner", on="snp")
    snps = pd.merge(tmpsnps, bim_3, how="inner", on="snp")

    G_1 = G_1[snps.i_x.values, :].compute()
    G_2 = G_2[snps.i_y.values, :].compute()
    G_3 = G_3[snps.i.values, :].compute()

    bim_1 = bim_1.iloc[snps.i_x.values].reset_index()
    bim_2 = bim_2.iloc[snps.i_y.values].reset_index()
    bim_3 = bim_3.iloc[snps.i.values].reset_index()

    LD_1, L_1, LDSCs_1, maf_1, p_1 = compute_ld(G_1)
    LD_2, L_2, LDSCs_2, maf_2, p_2 = compute_ld(G_2)
    LD_3, L_3, LDSCs_3, maf_3, p_3 = compute_ld(G_3)

    return LD_1, L_1, LDSCs_1, bim_1, maf_1, p_1, LD_2, L_2, LDSCs_2, bim_2, maf_2, p_2, LD_3, L_3, LDSCs_3, bim_3, maf_3, p_3
...
def sim_beta(model, share_model, eqtl_h2, n_snps):
    ...
    if share_model != "shared":
        ...
        b_qtls_3[c_qtls] = np.random.normal(loc=0, scale=np.sqrt(eqtl_h2 / n_qtls), size=n_qtls)
    else:
        ...
        b_qtls_3[c_qtls] = b_qtls[c_qtls]

    return b_qtls, b_qtls_2, b_qtls_3
...
def sim_gwas(L, ngwas, b_qtls, var_explained, alpha=None):
    """
...
def sim_eqtl(L, nqtl, b_qtls, eqtl_h2, return_ld = True):
    """
...
def sim_eqtl_proxy(L, nqtl, b_qtls, rho_tissue, eqtl_h2, return_ld = True):
    """
