...
# Only modify sim_beta and gen_ld functions
exec(open('/project/nmancuso_8/zeyunlu/projects/mefocus/scripts/focus.functions_3pop.py').read())
...
def main(args):
    ...
    argp.add_argument("prefix_pop3", help="Prefix to PLINK-formatted data for population 3")
    ...
    argp.add_argument("--ngwas_pop3", default=100000, type=int, help="Sample size for GWAS panel for pop 3")
    ...
    argp.add_argument("--nqtl_pop3", default=500, type=int, help="Sample size for eQTL panel for pop 3")

    argp.add_argument("--model", choices=["10pct", "1pct", "1snp", "pois2"], default="10pct",
                      help="SNP model for generating gene expression. 10pct = 10%% of SNPs, 1pct = 1%% of SNPs, 1snp = 1 SNP, pois2 = Drawn from a truncated Poisson distribution of mean 2 and a minimum possible value of 1")
    ...
    LD_pop1, L_pop1, LDSCs_pop1, bim_1, maf_1, p_pop1, LD_pop2, L_pop2, LDSCs_pop2, bim_2, maf_2, p_pop2, LD_pop3, L_pop3, LDSCs_pop3, bim_3, maf_3, p_pop3 = gen_ld(args.prefix_pop1, args.prefix_pop2, args.prefix_pop3)
    #b_qtls_pop1, b_qtls_pop2 = sim_beta(args.model, args.share_model, args.rhog, args.eqtl_h2, p_pop1)
    # Question: use p_pop1
    b_qtls_pop1, b_qtls_pop2, b_qtls_pop3 = sim_beta(args.model, args.share_model, args.eqtl_h2, p_pop1)


    # simulate GWAS under assumption that expression => downstream trait
    gwas_pop1, alpha_pop1 = sim_gwas(L_pop1, args.ngwas_pop1, b_qtls_pop1, args.var_explained)
    gwas_pop2, alpha_pop2 = sim_gwas(L_pop2, args.ngwas_pop2, b_qtls_pop2, args.var_explained, alpha_pop1)
    gwas_pop3, alpha_pop3 = sim_gwas(L_pop3, args.ngwas_pop3, b_qtls_pop3, args.var_explained, alpha_pop1)

    # meta analysis
    gwas_pop1["weights"] = 1 / gwas_pop1.se.values ** 2
    gwas_pop2["weights"] = 1 / gwas_pop2.se.values ** 2
    gwas_pop3["weights"] = 1 / gwas_pop3.se.values ** 2
    gwas_meta = gwas_pop1.copy(deep=True)
    gwas_meta.beta = ((gwas_pop1.weights * gwas_pop1.beta) + (gwas_pop2.weights * gwas_pop2.beta) + (gwas_pop3.weights * gwas_pop3.beta)) / (gwas_pop1.weights.values + gwas_pop2.weights.values + gwas_pop3.weights.values)
    gwas_meta.se = np.sqrt(1 / (gwas_pop1.weights.values + gwas_pop2.weights.values + gwas_pop3.weights.values))
    ...
    eqtl_pop3, coef_pop3, LD_qtl_pop3 = sim_eqtl(L_pop3, args.nqtl_pop3, b_qtls_pop3, args.eqtl_h2)
    ...
    if np.isclose(np.var(coef_pop3), 0):
        coef_pop3[np.random.choice(int(p_pop3), 2, replace=False)] = 1e-6
    ...
    score_pop3, within_var_pop3 = compute_twas(gwas_pop3, coef_pop3, LD_qtl_pop3)
    ...
    output_pop3, min_p_val, mean_chi2, med_chi2, z_twas_pop3, p_twas_pop3 = compute(gwas_pop3, eqtl_pop3, score_pop3, within_var_pop3, bim_3, maf_3, LDSCs_pop3, b_qtls_pop3, alpha_pop3, coef_pop3)

    zscores_pop1 = [z_twas_pop1]
    zscores_pop2 = [z_twas_pop2]
    zscores_pop3 = [z_twas_pop3]
    ...
    for idx in range(args.ngenes - 1):
        ...
        eqtl_pop3, coef_pop3, _ = sim_eqtl(L_pop3, args.nqtl_pop3, b_qtls_pop3, args.eqtl_h2, return_ld = False)
        ...
        z_twas_pop3, p_twas_pop3 = calc_twas(score_pop3, within_var_pop3)
        ...
        zscores_pop3.append(z_twas_pop3)
        zscores_meta.append(z_twas_meta)

    wmats = [np.array(wgt_pop1).T, np.array(wgt_pop2).T, np.array(wgt_pop3).T]
    zscores = [np.array(zscores_pop1), np.array(zscores_pop2), np.array(zscores_pop3)]
    zscores_meta = np.array(zscores_meta)
    ldmats = [LD_qtl_pop1, LD_qtl_pop2, LD_qtl_pop3]
    ...
    V_pop3 = mdot([np.array(wgt_pop3), LD_qtl_pop3, np.array(wgt_pop3).T]) # calculate gene covariance matrix from weight and LD matrices
    V_pop3_scaled = np.diag(1 / np.sqrt(np.diag(V_pop3))) # scale this matrix
    cor_pop3 = mdot([V_pop3_scaled,  V_pop3,  V_pop3_scaled])
    prior_chisq_pop3 = mdot([zscores[2], linalg.pinv(cor_pop3), zscores[2]])

    unbiased_prior_chisq_pop1=prior_chisq_pop1 - args.ngenes
    unbiased_prior_chisq_pop2=prior_chisq_pop2 - args.ngenes
    unbiased_prior_chisq_pop3=prior_chisq_pop3 - args.ngenes

    prior_chisq = [unbiased_prior_chisq_pop1, unbiased_prior_chisq_pop2, unbiased_prior_chisq_pop3]

    if (unbiased_prior_chisq_pop1 < 0):
        prior_chisq[0] = prior_chisq_pop1
    if (unbiased_prior_chisq_pop2 < 0):
        prior_chisq[1] = prior_chisq_pop2
    if (unbiased_prior_chisq_pop3 < 0):
        prior_chisq[2] = prior_chisq_pop3

    pval_p1 = stats.distributions.chi2.sf(zscores[0]**2, df=1)
    pval_p2 = stats.distributions.chi2.sf(zscores[1]**2, df=1)
    pval_p3 = stats.distributions.chi2.sf(zscores[2]**2, df=1)
    ...
    pips_p3, null_res_p3 = fm.fine_map([zscores[2]], [wmats[2]], [ldmats[2]], prior_prob=prior_prob, prior_chisq=[prior_chisq[2]])
    ...
    output = pd.DataFrame({"sim": args.sim,
                           ...
                           "zscores.3": [0] + list(zscores[2]),
                           "p_val.3": [1] + list(pval_p3),
                           ...
                           "pop3.pip": [null_res_p3] + list(pips_p3),
